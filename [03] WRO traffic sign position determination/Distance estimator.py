...
FOCAL_LENGTH_PX = 621  # Focal length in pixels 
...
cv2.namedWindow("General HSV Trackbars", cv2.WINDOW_NORMAL) #cv2.WINDOW_NORMAL - It’s a window flag that allows you to manually resize the OpenCV window create
...
except cv2.error as e: #This saves the exception object (the error message) into a variable e, so you can print or log it.
...
SAFE_ZONE_LEFT_EDGE_X = SAFE_ZONE_CENTER_X - (SAFE_ZONE_WIDTH_PX // 2) # // -> floor division
